...
cdef uint64_t PATH_UNSET = <uint64_t> 0b00
cdef uint64_t PATH_LEFT  = <uint64_t> 0b10
cdef uint64_t PATH_RIGHT = <uint64_t> 0b01
cdef uint64_t PATH_STOP  = <uint64_t> 0b11

cdef intp_t FLAG_UNSET           = <intp_t> 0
cdef intp_t FLAG_DETECTED_CHANGE = <intp_t> 1
cdef intp_t FLAG_ALREADY_CHECKED = <intp_t> 2

cdef intp_t MASK_UNSET = <intp_t> 0
cdef intp_t MASK_LEFT  = <intp_t> 1
cdef intp_t MASK_RIGHT = <intp_t> 2

cdef intp_t DEPTH_UNSET = <intp_t> (-1)
cdef intp_t NODEID_UNSET = <intp_t> (-1)
cdef double EPSILON = <double> np.finfo(np.float64).eps

cdef double SPLIT_NO_CHANGE = <double> 2.0    # This is used to indicate that the split cannot change by any mean, thus it is not possible to improve the impurity.
cdef double SPLIT_UNSET     = <double> (-1.0) # This is used to indicate that the split is not set, thus even a split with 0.0 impurity improvement is better than this.
...
cdef inline void set_path_step(uint64_t[::1] paths, intp_t instance, intp_t step, uint64_t direction) noexcept nogil:
    cdef uint64_t block = paths[instance] # type: ignore
    cdef uint64_t value = block & (0b11 << (step * 2))

    # If the path is unset, then we can set the direction directly
    # If otherwise the direction is STOP but the path is not unset, then we can overwrite the direction
    if value == PATH_UNSET or direction == PATH_STOP:
        block = block | (direction << (step * 2))

    # Update the path
    paths[instance] = block # type: ignore
...
cdef class Splitter:
    ...
    cdef int init(
        ...
        self.bestsplitbuffer = np.zeros(n_samples, dtype=np.intp) # type: ignore
        self.splitbuffer = np.zeros(n_samples, dtype=np.intp) # type: ignore
        self.paths = np.zeros(n_samples, dtype=np.uint64) # type: ignore
...
cdef inline int filter_out_fast(
    Splitter splitter,
    ...
    # TODO: Implement fast filter out function, to reduce the time complexity from O(n^2) to O(n)
    return 0


cdef inline int filter_out_slow(
    Splitter splitter,
    Partitioner partitioner,
    SplitRecord* best_split,
    intp_t curr_pos,
    ...
    cdef intp_t[::1] original = splitter.bestsplitbuffer
    cdef intp_t[::1] current  = splitter.samples
    
    cdef intp_t osample, csample
    cdef intp_t ostart, cstart
    cdef intp_t oend, cend
    cdef double y, cls
    cdef intp_t flag
    cdef int i, j

    cls = <double> 1.0 if target == <bint> 1 else <double> 0.0

    # Calculate the range of samples that matches the correct side of the original best split
    if best_left == <bint> 1:
        ostart = splitter.start
        oend = best_split.pos
    else:
        ostart = best_split.pos
        oend = splitter.end - partitioner.n_missing

    # Calculate the range of samples that matches the correct side of the alternative split
    if curr_left == <bint> 1:
        cstart = splitter.start
        cend = curr_pos
    else:
        cstart = curr_pos
        cend = splitter.end - partitioner.n_missing


    # Iterate through the correct side of the alternative split
    for i in range(cstart, cend):
        # Select the sample
        csample = <intp_t> current[i]  # type: ignore
        y = <double> splitter.y[csample, 0]  # type: ignore
        
        # And check that the label matches the target label
        if y != cls:
            continue

        # Then iterate through the correct side of the original best split
        for j in range(ostart, oend):
            osample = <intp_t> original[j]  # type: ignore

            # If the sample ids match, stop the stable path, as the node that has been reached will change upon label change.
            if csample == osample:
                set_path_step(splitter.paths, osample, splitter.current_depth, PATH_STOP)

    return 0

cdef inline bint equal(double a, double b) noexcept nogil:
    # Check if a is equal to be between a range of b-EPSILON and b+EPSILON
    return <bint> ((a >= b-EPSILON) and (a <= b+EPSILON))

cdef inline bint less_than(double a, double b) noexcept nogil:
    # Check if a is less than b
    return <bint> (a+EPSILON < b-EPSILON)
  
cdef inline int update_stable_paths(
    Splitter splitter,
    Partitioner partitioner,
    Criterion criterion,
    ParentInfo* parent_record,
    ...
    cdef intp_t n_constant_features = parent_record.n_constant_features
    cdef intp_t start = splitter.start
    cdef intp_t end = splitter.end
    cdef intp_t end_non_missing
    cdef intp_t n_left, n_right
    cdef intp_t p, p_prev
    cdef intp_t instance
    cdef intp_t feature

    cdef float[::1] feature_values
    cdef float threshold
    
    cdef bint tiebreak
    cdef int fi, i

    cdef double curr_split_lpg
    cdef double curr_split_lng
    cdef double curr_split_rpg
    cdef double curr_split_rng

    cdef double curr_split_gain
    cdef double curr_split_limp
    cdef double curr_split_rimp

    # Then we need to store to the splitbuffer the current best split sorted samples
    memcpy(&splitter.splitbuffer[start], &splitter.samples[start], sizeof(intp_t) * (end - start)) # type: ignore

    for fi in range(0, splitter.n_features):
        feature = splitter.features[fi] # type: ignore
        partitioner.sort_samples_and_feature_values(feature) # type: ignore
        end_non_missing = end - partitioner.n_missing
        feature_values = splitter.feature_values

        if (end_non_missing == start or feature_values[end_non_missing - 1] <= feature_values[start] + FEATURE_THRESHOLD): # type: ignore
            continue
        ...
        if partitioner.n_missing > 0:
            printf("WARNING: Missing values are not supported in the update_stable_paths function.")

        p = start
        while p < end:
            partitioner.next_p(&p_prev, &p) # type: ignore

            # If we exceed the end of the non-missing values, we skip
            if p >= end_non_missing:
                continue

            # Check that we are not evaluating the same split as the new flipped best split
            threshold = feature_values[p] # type: ignore
            if feature == best_split.feature and threshold == best_split.threshold:
                continue
            
            # Update the criterion with the new position
            criterion.update(p)
            n_left = p - start
            n_right = end_non_missing - p + partitioner.n_missing

            # Reject if min_samples_leaf is not guaranteed
            if n_left < splitter.min_samples_leaf or n_right < splitter.min_samples_leaf:
                continue

            # Calculate the impurity of the current split
            criterion.children_impurity(&curr_split_limp, &curr_split_rimp)
            curr_split_gain = criterion.impurity_improvement(parent_record.impurity, curr_split_limp, curr_split_rimp)

            # Set the tiebreak flag such that a new best split can occur only alphanumerically after the current best split
            tiebreak = <bint>(splitter.features[fi] > best_split.feature) # type: ignore
            tiebreak = tiebreak or (
                feature_values[p] > best_split.threshold and # type: ignore
                splitter.features[fi] == best_split.feature) # type: ignore
            tiebreak = tiebreak and equal(curr_split_gain, best_split.improvement)
            # tiebreak = <bint> False

            # Initialize the G* values to SPLIT_UNSET
            curr_split_lpg = SPLIT_UNSET
            curr_split_rpg = SPLIT_UNSET
            curr_split_lng = SPLIT_UNSET
            curr_split_rng = SPLIT_UNSET

            # Update the G* values only if a flip can be performed in each of the 4 possible cases
            if (criterion.can_flip(<bint> 1, <bint> 1)): curr_split_lpg = criterion.gstar(<bint> 1, <bint> 1)
            if (criterion.can_flip(<bint> 0, <bint> 1)): curr_split_rpg = criterion.gstar(<bint> 0, <bint> 1)
            if (criterion.can_flip(<bint> 1, <bint> 0)): curr_split_lng = criterion.gstar(<bint> 1, <bint> 0)
            if (criterion.can_flip(<bint> 0, <bint> 0)): curr_split_rng = criterion.gstar(<bint> 0, <bint> 0)


            # Check the 4 possible cases of a positive flip
            # To the left of both splits
            if (splitter.best_lpg < curr_split_lpg) or (equal(splitter.best_lpg, curr_split_lpg) and not tiebreak):
                filter_out_slow(splitter, partitioner, best_split, p, <bint> 1, <bint> 1, <bint> 1)

            # To the left of the current split and to the right of the best one
            if (splitter.best_rpg < curr_split_lpg) or (equal(splitter.best_rpg, curr_split_lpg) and not tiebreak):
                filter_out_slow(splitter, partitioner, best_split, p, <bint> 0, <bint> 1, <bint> 1)

            # To the right of the current split and to the left of the best one
            if (splitter.best_lpg < curr_split_rpg) or (equal(splitter.best_lpg, curr_split_rpg) and not tiebreak):
                filter_out_slow(splitter, partitioner, best_split, p, <bint> 1, <bint> 0, <bint> 1)

            # To the right of both splits
            if (splitter.best_rpg < curr_split_rpg) or (equal(splitter.best_rpg, curr_split_rpg) and not tiebreak):
                filter_out_slow(splitter, partitioner, best_split, p, <bint> 0, <bint> 0, <bint> 1)

            # Check the 4 possible cases of a negative flip
            # To the left of both splits
            if (splitter.best_lng < curr_split_lng) or (equal(splitter.best_lng, curr_split_lng) and not tiebreak):
                filter_out_slow(splitter, partitioner, best_split, p, <bint> 1, <bint> 1, <bint> 0)

            # To the left of the current split and to the right of the best one
            if (splitter.best_rng < curr_split_lng) or (equal(splitter.best_rng, curr_split_lng) and not tiebreak):
                filter_out_slow(splitter, partitioner, best_split, p, <bint> 0, <bint> 1, <bint> 0)

            # To the right of the current split and to the left of the best one
            if (splitter.best_lng < curr_split_rng) or (equal(splitter.best_lng, curr_split_rng) and not tiebreak):
                filter_out_slow(splitter, partitioner, best_split, p, <bint> 1, <bint> 0, <bint> 0)

            # To the right of both splits
            if (splitter.best_rng < curr_split_rng) or (equal(splitter.best_rng, curr_split_rng) and not tiebreak):
                filter_out_slow(splitter, partitioner, best_split, p, <bint> 0, <bint> 0, <bint> 0)
            
    # Restore the original split to the splitter
    memcpy(&splitter.samples[start], &splitter.splitbuffer[start], sizeof(intp_t) * (end - start)) # type: ignore

    # Iterate for every instance of the split and update their path steps
    for i in range(start, end):
        instance = splitter.bestsplitbuffer[i] # type: ignore
        if i < best_split.pos:
            set_path_step(splitter.paths, instance, splitter.current_depth, PATH_LEFT)
        else:
            set_path_step(splitter.paths, instance, splitter.current_depth, PATH_RIGHT)
...
cdef inline int node_split_best(
        ...
        for i in range(n_searches):
            ...
            while p < end_non_missing:
                ...
                if (current_proxy_improvement > best_proxy_improvement) or (
                    ...
                    current_split.feature < best_split.feature # type: ignore
                ):
                    ...
                    # Since we found a best split candidate, we want to save it into the bestsplitbuffer
                    # to be able to restore the original order of the samples after the split
                    memcpy(&splitter.bestsplitbuffer[start], &splitter.samples[start], sizeof(intp_t) * (end - start)) # type: ignore

                    # Initialize the G* values to SPLIT_NO_CHANGE
                    splitter.best_lpg = SPLIT_NO_CHANGE
                    splitter.best_rpg = SPLIT_NO_CHANGE
                    splitter.best_lng = SPLIT_NO_CHANGE
                    splitter.best_rng = SPLIT_NO_CHANGE

                    # Update the G* values only if a flip is allowed in each of the 4 possible cases
                    if criterion.can_flip(<bint> 1, <bint> 1): splitter.best_lpg = criterion.gstar(<bint> 1, <bint> 1)
                    if criterion.can_flip(<bint> 0, <bint> 1): splitter.best_rpg = criterion.gstar(<bint> 0, <bint> 1)
                    if criterion.can_flip(<bint> 1, <bint> 0): splitter.best_lng = criterion.gstar(<bint> 1, <bint> 0)
                    if criterion.can_flip(<bint> 0, <bint> 0): splitter.best_rng = criterion.gstar(<bint> 0, <bint> 0)
    ...
    if splitter.compute_stable_paths:
        update_stable_paths(splitter, partitioner, criterion, parent_record, &best_split)
