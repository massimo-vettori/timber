...
from ..utils._typedefs cimport float32_t, float64_t, intp_t, int8_t, int32_t, uint32_t, uint64_t
...
cdef class Splitter:
    ...
    cdef public intp_t current_depth     # Current depth of the tree
    cdef public intp_t max_depth         # Max depth of the tree
    
    cdef public bint          compute_stable_paths # A flag that if activated will allow the tree to calculate the stable path prefixes
    cdef public intp_t[::1]   bestsplitbuffer # Buffer for the best split
    cdef public intp_t[::1]   splitbuffer
    cdef public uint64_t[::1] paths # Stable prefixes of the paths of the tree

    cdef public double best_lpg
    cdef public double best_rpg
    cdef public double best_lng
    cdef public double best_rng
