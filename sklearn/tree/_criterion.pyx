...
cdef class Criterion:
    ...
    cdef double gstar(self, bint left, bint target) noexcept nogil:
        pass

    cdef bint can_flip(self, bint left, bint target) noexcept nogil:
        pass

    cdef bint can_change_leaf(self, bint target) noexcept nogil:
        pass
...
cdef class ClassificationCriterion(Criterion):
    ...
    cdef bint can_flip(self, bint left, bint target) noexcept nogil:
        cdef intp_t cls = (<intp_t> 0) if target == (<bint> 0) else (<intp_t> 1)
        cdef double total, lside, rside
        total = self.sum_total[0, cls] # type: ignore
        lside = self.sum_left[0, cls]  # type: ignore
        rside = self.sum_right[0, cls] # type: ignore

        if total < 1:
            return <bint> False

        if left == <bint> True and lside < 1:
            return <bint> False

        if left == <bint> False and rside < 1:
            return <bint> False
    ...
    cdef bint can_change_leaf(self, bint target) noexcept nogil:
        ...
        yval = <intp_t> 1 if target == <bint> True else <intp_t> 0
        tgt = self.sum_total[0, yval] # type: ignore
        oth = self.sum_total[0, 1-yval] # type: ignore

        if tgt < 1:
            return <bint> False

        if (tgt == oth) or (tgt == oth+1):
            return <bint> True
    ...
    cdef double gstar(self, bint left, bint target) noexcept nogil:
        ...
        cls = <intp_t> 1 if target == <bint> True else <intp_t> 0

        # Updates the current counts of the parent node of the split
        # The target class will decrease by 1, since one of its instances will have its label flipped to the other class
        # The other class as a consequence will increase by 1
        self.sum_total[0, cls]   -= <double> 1.0 # type: ignore
        self.sum_total[0, 1-cls] += <double> 1.0 # type: ignore

        # The same pattern is repeated either to the left counts or the right counts, depending on the
        # direction of the label flip attack
        if left:
            self.sum_left[0, cls]   -= <double> 1.0 # type: ignore
            self.sum_left[0, 1-cls] += <double> 1.0 # type: ignore
        else:
            self.sum_right[0, cls]   -= <double> 1.0 # type: ignore
            self.sum_right[0, 1-cls] += <double> 1.0 # type: ignore

        # Now we calculate the parent node impurity and the "flipped gain" of the attack,
        # in other words the first member of the G* triple
        root = self.node_impurity()
        self.children_impurity(&limp, &rimp)
        gain = self.impurity_improvement(root, limp, rimp)

        # Revert the changes made to the counts, starting from the child node that was affected by the attack
        # and then to the parent node
        if left:
            self.sum_left[0, cls]   += <double> 1.0 # type: ignore
            self.sum_left[0, 1-cls] -= <double> 1.0 # type: ignore
        else:
